...
cd MastersThesis/application/image-store-service
npm install --silent > /dev/null 2>&1
echo "Installed application dependencies."

# Create systemctl service
echo "Creating systemctl service..."
sudo bash -c 'cat << EOF > /etc/systemd/system/image-store-service.service
[Unit]
Description=Image Store Service
After=network.target

[Service]
User=ubuntu
EnvironmentFile=/etc/environment
ExecStart=/usr/bin/node /home/ubuntu/MastersThesis/application/image-store-service/app.js
WorkingDirectory=/home/ubuntu/MastersThesis/application/image-store-service
Restart=always
StandardOutput=file:/var/log/image-store-service.log
StandardError=file:/var/log/image-store-service.log
        ...
        "logs": {
                "logs_collected": {
                        "files": {
                                "collect_list": [
                                        {
                                                "file_path": "/var/log/image-store-service.log",
                                                "log_group_name": "/aws/ec2/masters-thesis/image-store-service",
